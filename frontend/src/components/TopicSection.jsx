// One topic's chart block: metric pills, trend chart, legend, expand
// modal, CSV export, data table, suppression + break notes. Shared by the
// district page (primary = district, peers selectable) and the school
// page (primary = school, the district itself rides as the one "peer").
...
// subgroupCode: which subgroup file backs the "Break out by" pills —
// null disables them (school pages: subgroup files are district-level).
// modalSubtitle: expanded-chart subtitle; defaults to the district form.
export default function TopicSection({ topic, doc, stateDoc, peerDocs, peerColorOf,
                                       subgroupCode, modalSubtitle }) {
  ...
  const groupsEnabled = DIMENSIONS.length > 0 && subgroupCode != null

  useEffect(() => {
    if (!dimension || subDoc || !groupsEnabled) return
    setSubError(false)
    let alive = true
    loadSubgroups(subgroupCode).then(
      (d) => alive && setSubDoc(d),
      (err) => {
        console.error(err)
        if (alive) {
    ...
    return () => { alive = false }
  }, [dimension, subDoc, subgroupCode, groupsEnabled])
    ...
    <section className="topic-section" id={topic.id}>
      ...
      {expanded && (
        <ChartModal
          ...
          subtitle={modalSubtitle ?? `${doc.district.dpi_name} School District · ${topic.sublabel}`}
